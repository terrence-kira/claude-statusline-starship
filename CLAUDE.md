...
**Worktree-aware path segment.** When Claude Code reports `workspace.git_worktree`, the dirname is suppressed (it duplicates the worktree slug) and the implicit `user/` prefix is stripped from the branch (worktree branches follow `<user>/<scope>`). `truncate_middle` keeps the head and tail of long branch / dir names recognizable. Constants live at top of the dir/branch block: `DIR_CAP=24`, `BRANCH_CAP=28`.
